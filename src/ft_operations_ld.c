...
/**/
	if (parameters[0] == T_DIR)
		proc->regestries[proc->arena[(proc->pc + 4) % IDX_MOD]] = ft_reverse_endianness(proc->arena + proc->pc, 4);
	else
		proc->regestries[proc->arena[(proc->pc + 2) % IDX_MOD]] = temp + \
						(ft_reverse_endianness(proc->arena + proc->pc, 2) % IDX_MOD);
...
/**/
	if (parameters[0] == T_DIR)
		proc->regestries[proc->arena[(proc->pc + 4) % IDX_MOD]] = \
									ft_reverse_endianness(proc->arena + proc->pc, 4);
	else
		proc->regestries[proc->arena[(proc->pc + 2) % IDX_MOD]] = temp + \
						ft_reverse_endianness(proc->arena + proc->pc, 2);
	proc->carry = (proc->regestries[proc->arena[proc->pc++]] == 0) ? 1 : 0;
/**/
	proc->pc = (proc->pc + \
					ft_sizeof_params(proc, parameters)) % MEM_SIZE;
...
/**/
	if (parameters[0] == T_IND)
		temp = temp + (ft_reverse_endianness(proc->arena + proc->pc, 4) % IDX_MOD);
	else
		temp = temp + ((ft_reverse_endianness(proc->arena + proc->pc, ft_size(parameters[1], 2)) +\
			ft_reverse_endianness(proc->arena + proc->pc, ft_size(parameters[2], 2))) % IDX_MOD);
	proc->regestries[proc->arena[proc->pc +  ft_size(parameters[0], 2)]] = temp;
/**/
	proc->pc = (proc->pc + \
					ft_sizeof_params(proc, parameters)) % MEM_SIZE;
}
...
/**/
	if (parameters[0] == T_IND)
		...
		temp = temp + (ft_reverse_endianness(proc->arena + proc->pc, ft_size(parameters[1], 2)) +\
				ft_reverse_endianness(proc->arena + proc->pc, ft_size(parameters[2], 2)));
	proc->regestries[proc->arena[proc->pc + ft_size(parameters[0], 2)]] = temp;
	proc->carry = (proc->regestries[proc->arena[proc->pc]] == 0) ? 1 : 0;
/**/
	proc->pc = (proc->pc + \
